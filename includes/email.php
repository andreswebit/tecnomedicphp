...
// Si fecha/hora vienen null (dato viejo o corrupto), usa un texto de reemplazo
// en vez de dejar que PHP tire un TypeError por pasarle null a un string.
function _fh(?string $valor, string $fallback): string {
...
function email_confirmacion(string $nombre, string $email, ?string $fecha, ?string $hora): void {
    $fecha = _fh($fecha, 'a confirmar');
    $hora  = _fh($hora, 'a confirmar');
